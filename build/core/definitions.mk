...
define ev-build-file-to-bitcode
...
_FLAGS := $$($$(my)CFLAGS) \
          ...
          $$(call host-c-includes,$$($(my)C_INCLUDES)) \
					-c
LOCAL_CFLAGS := $$(LOCAL_CFLAGS:-D%=-D %)
_FLAGS := $$(call host-c-includes,$$(LOCAL_C_INCLUDES) $$(LOCAL_PATH)) \
          $$(LOCAL_CFLAGS) \
          $$(call host-c-includes,$$($(my)C_INCLUDES))
_TEXT := "Compile BC   $$(call get-src-file-text,$1)"
_CC   := $$(TARGET_CC)

$$(eval $$(call ev-build-file-to-bitcode))
LOCAL_OBJECTS += $$(_OBJ)
endef

define  ev-compile-cpp-source-to-bitcode
_SRC:=$$(LOCAL_PATH)/$(1)
_OBJ:=$$(LOCAL_OBJS_DIR)/$(2)
_FLAGS := $$($$(my)CXXFLAGS) \
          $$(call get-src-file-target-cflags,$(1)) \
...
compile-c-source-to-bitcode = $(eval \
    $(call ev-compile-c-source-to-bitcode,$1,$(1:%.c=%.bc)))
compile-cpp-source-to-bitcode = $(eval \
    $(call ev-compile-cpp-source-bitcode,$1,$(1:%$(LOCAL_CPP_EXTENSION)=%.bc)))
